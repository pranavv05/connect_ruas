 ...
 * Usage: node scripts/update-specific-user.js <userId> <newEmail> <newFullName>
 */
...
async function updateSpecificUser() {
  ...
  if (args.length < 3) {
    console.log('Usage: node scripts/update-specific-user.js <userId> <newEmail> <newFullName>');
    console.log('Example: node scripts/update-specific-user.js user_123 john@example.com "John Doe"');
    process.exit(1);
  }
